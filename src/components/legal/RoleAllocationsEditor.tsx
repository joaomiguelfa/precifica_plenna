...
export function RoleAllocationsEditor({ roles, assumptions, onChange, hoursLabel = 'Horas estimadas' }: Props) {
  function addRole() {
  ...
  const totalLaborCost = roles.reduce((sum, r) => sum + r.hours * hourlyCostForRole(r.role, assumptions), 0)
  ...
  return (
    <div className="space-y-2">
      ...
      {roles.map((r, index) => {
        const hourlyCost = hourlyCostForRole(r.role, assumptions)
        return (
          <div key={r.id} className="grid grid-cols-12 items-end gap-2 rounded-md bg-slate-50 p-2 dark:bg-slate-800/60">
            <label className="col-span-12 block text-sm sm:col-span-6">
              ...
              <select
                value={r.role}
                onChange={(e) => updateRole(index, { role: e.target.value as LawyerRole })}
